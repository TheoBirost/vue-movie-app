...
<template>
  <div class="universal-card p-6">
    ...
    <div v-if="loading" class="text-center py-10">
      ...
      <p class="text-[var(--color-ink-soft)] text-sm">Chargement des acteurs...</p>
    </div>
    ...
    <div v-else class="overflow-x-auto">
      <table class="w-full text-left border-collapse">
        ...
        <tbody>
          <tr v-for="actor in actors" :key="actor.id" class="border-b border-[var(--color-rule)] hover:bg-[var(--color-paper-sunk)] transition-colors">
            <td class="p-4 text-[var(--color-ink)] font-medium">{{ actor.firstname }} {{ actor.lastname }}</td>
            <td class="p-4 text-right">
              <button @click="editActor(actor)" class="text-[var(--color-night)] hover:text-[var(--color-ink)] text-xs font-bold uppercase tracking-wider transition-colors border border-[var(--color-rule)] hover:bg-[var(--color-paper-sunk)] px-3 py-1.5 rounded mr-2">
                Modifier
              </button>
              <button @click="confirmDelete(actor)" class="text-[var(--color-danger)] hover:text-[var(--color-danger)] text-xs font-bold uppercase tracking-wider transition-colors border border-[var(--color-danger)]/40 hover:bg-[var(--color-danger)]/10 px-3 py-1.5 rounded">
                Supprimer
              </button>
            </td>
          </tr>
