...
<template>
  <div class="universal-card p-6">
    <div class="flex justify-between items-center mb-6">
      <h2 class="garamond text-3xl font-bold text-white">Gestion des Réalisateurs</h2>
      ...
      <table class="w-full text-left border-collapse">
        ...
        <tbody>
          <tr v-for="director in directors" :key="director.id" class="border-b border-border hover:bg-white/5 transition-colors">
            <td class="p-4 text-white font-medium">{{ director.firstname }} {{ director.lastname }}</td>
            <td class="p-4 text-right">
              <button @click="editDirector(director)" class="text-gold hover:text-white text-xs font-bold uppercase tracking-wider transition-colors border border-gold/30 hover:bg-gold/10 px-3 py-1.5 rounded mr-2">
                Modifier
              </button>
