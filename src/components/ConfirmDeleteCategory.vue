    ...
    <div class="confirm-dialog bg-[#16181E] border border-[#2A2D36] rounded-lg w-full max-w-md shadow-2xl">
      ...
      <main class="p-6">
        <p class="text-[#C1C1C7]">
          ...
          <strong class="text-white">{{ props.category?.name }}</strong> ? Cette action est irréversible.
        </p>
      ...
      <footer class="p-6 flex justify-end gap-4 bg-[#0d0d0f]/50 rounded-b-lg">
        ...
        <button @click="emit('confirm')" :disabled="props.category?.moviesCount > 0" class="px-6 py-2.5 rounded-lg text-sm font-bold text-white bg-red-600 hover:bg-red-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed">
          Supprimer
        </button>
      </footer>
    </div>
