...
watch(isVisible, (newValue) => {
  if (newValue) {
    ...
    gsap.fromTo(modalElement.value,
        { opacity: 0, scale: 0.8 },
        { opacity: 1, scale: 1, duration: 0.4, ease: 'power3.out', delay: 0.1 }
    )
  } else {
    gsap.to(modalElement.value,
...
<template>
  ...
  >
    ...
    <div
        ref="modalElement"
        class="relative w-full max-w-md bg-[#16181E] rounded-lg shadow-2xl border border-[#FFD700]/30 p-8 text-center"
    >
      <div class="flex flex-col items-center">
        ...
        <button
            @click="close"
            class="px-8 py-2 bg-[#FFD700] text-black font-bold rounded-lg hover:bg-[#FFE55C] transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-[#16181E] focus:ring-[#FFD700]"
        >
          Fermer
        </button>
      </div>
