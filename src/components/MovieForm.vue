...
const saveMovie = async () => {
  ...
  try {
    const actorsIris = selectedActors.value.map(id => `/api/actors/${id}`)

    if (props.movie?.id) {
      await api.patch(`/movies/${props.movie.id}`, {
        name: title.value,
    ...
    >
      ...
      <div class="space-y-5">
        ...
        <textarea
            ...
            class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl h-28 resize-none focus:ring-2 focus:ring-blue-500 focus:outline-none transition-all"
        ></textarea>
        ...
        <div>
          ...
          <div class="bg-gray-50 border border-gray-200 rounded-xl p-3 max-h-48 overflow-y-auto space-y-2">
            ...
            : 'bg-white border-gray-300'"
                >
                  <svg
                      v-if="selectedActors.includes(actor.id)"
